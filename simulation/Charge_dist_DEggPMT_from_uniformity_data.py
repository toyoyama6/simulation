...
class Charge_dist_DEggPMT():
    ...
    def __init__(self, PMTID=-1, isDebug=False):
        ...
        if(PMTID==-1):
            ...
            for iPMT in range(len(self.listPMT)):
                ...
                MPE_val[iPMT] = dfnp[0,].reshape((self.nMPE_azi, -1)) 
            MPE_val = np.average(MPE_val, axis=0)
            df = pd.DataFrame(MPE_val)
            # df.to_hdf("./theta_pde.hdf5", key = "df")
            plt.plot(df.T)
            plt.show()
            ...
            if(isDebug): 
                ...
                cc = ax.contourf(self.MPE_azi*np.pi/180, self.MPE_zen, MPE_val.T, color = "jet")
                ...
                plt.tight_layout()
                plt.savefig("./AE_distribution_from_the_average_of_11PMTs_uniformity_data.pdf")
            ...
            MPE_val = dfnp[0,].reshape((self.nMPE_azi, -1)) 
            if(isDebug):
                X, Y = np.meshgrid(self.MPE_azi*np.pi/180, self.MPE_zen)
                ytick = [10, 20, 30, 40, 50, 60]
                fig, ax = plt.subplots(subplot_kw=dict(projection='polar'))
                # ax.pcolormesh(self.MPE_azi*np.pi/180, self.MPE_zen, MPE_val.T)
                cc = ax.contourf(self.MPE_azi*np.pi/180, self.MPE_zen, MPE_val.T)
                plt.title("SQ{0:0>4}".format(PMTID), fontsize = 25)
                # plt.title("AE distribution", fontsize = 25)
                plt.xticks(fontsize = 20)
                plt.yticks(ytick, [str(i) + chr(176) for i in ytick], fontsize = 11)
                clb = plt.colorbar(cc)
                clb.ax.tick_params(labelsize = 15)
                plt.tight_layout()
                plt.savefig("./AE_distribution_from_one_PMT.pdf")
    ...
    def GetChgSigma(self, zen, phi):
        ...
        return self._GetChgSigma_fromIdx(zen*np.pi/180, idx_phi%4)*w1 \
                        + self._GetChgSigma_fromIdx(zen*np.pi/180, (idx_phi+1)%4)*(1-w1)
    ...
    def GetQ0(self, zen, phi):
        ...
        return self._GetQ0_fromIdx(zen*np.pi/180, idx_phi%4)*w1 \
                        + self._GetQ0_fromIdx(zen*np.pi/180, (idx_phi+1)%4)*(1-w1)
    ...
    def GetChargeAverage(self, zen, phi):
        ...
        bt = MPEfac*self.q0c*(self.tauref*self.alref/self.q0ref+1-self.alref)
    ...
    def _convertXYtoZenPhi(self, X, Y):
        ...
        if(rho>101):
            ...
            zen = np.arctan(rho/(np.sqrt(R2*R2-(rho-43.9106)**2)+59.5))*180.0/np.pi
        else:
...
if(__name__=="__main__"):
    ...
    chgdist = Charge_dist_DEggPMT(PMTID = 775, isDebug=True)
    ...
    #         # q_bar = None
    #         # try:
    #         #     q_bar = np.sum(PDF[(PDF>0)] * q) / np.sum(PDF[PDF>0])
    #         # except:
    ...
    # plt.title("AE map for SQ{0:0>4}".format(PMTID))
    # plt.colorbar(cc)
    # plt.show()
    # """
