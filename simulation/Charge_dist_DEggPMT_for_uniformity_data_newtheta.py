...
class Charge_dist_DEggPMT():
    ...
    def __init__(self, PMTID=-1, sys_err, isDebug=False):
        #### constructor #####################
        ...
        df = pd.read_hdf(f"./{sys_err}mm_interpolate1.hdf")
        self.nMPE_zen = 64
        self.nMPE_azi = 72
        self.MPE_zen = df.columns.array
        ...
        if(PMTID==-1):
            ...
            # df = df / df.max().max()
            # df.to_hdf(f"./theta_pde_{PMTID}.hdf5", key = "df")
            if(isDebug):
                plt.title(f"SQ{PMTID:0>4}", fontsize = 25)
                plt.xlabel(r"$\theta$", fontsize = 20)
                plt.xticks(fontsize = 15)
                plt.yticks(fontsize = 15)
                plt.plot(self.MPE_zen, MPE_val.T)
