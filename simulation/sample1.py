...
class Charge_dist_DEggPMT:
    def __init__(self, PMTID=-1, isDebug=False):
        ...
        def logistic_function(x, A, k, x0):
            return -0.6 / (1 + A * np.exp(-k * (x - x0))) + 0.98

        def interpolation(x, y):
            X = np.linspace(np.min(x), np.max(x), 30, endpoint=False).T
        ...
        df = df.to_numpy()
        data = df.T * logistic_function(Zen, p0[0], p0[1], p0[2]).T
        self.MPE_val = data.T
        x_list = []
        y_list = []
        for i in range(len(df)):
            x, y = interpolation([self.MPE_zen[-1], 90], [df[i, -1], df[i, -1] * 0.33])
            # plt.plot(x, y, color="royalblue", alpha=0.1)
            x_list.append(x)
        ...
        if isDebug:
            ...
            plt.plot(self.MPE_zen, self.MPE_val.T)
            # clb = plt.colorbar(cc)
            # clb.ax.tick_params(labelsize = 15)
            plt.tight_layout()
            plt.savefig(f"./uniformity_data_of_my_function2.pdf")
            plt.close()
...
if __name__ == "__main__":
    chgdist = Charge_dist_DEggPMT(PMTID=-1, isDebug=True)
    ...
    plt.colorbar(cc)
    plt.savefig(f"AE_map_my_function2.pdf")
